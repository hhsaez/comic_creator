...
void cutImage( const std::filesystem::path &path, const std::filesystem::path &outDir, const Settings &settings )
{
...
Path createOnlineImages( const PathArray &pages, const Settings &settings ) noexcept
{
...
Path createPrintingImages( const PathArray &pages, const Settings &settings ) noexcept
{
	if ( pages.size() % 4 != 0 ) {
